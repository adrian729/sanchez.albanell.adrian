...
What ends up in dist/:
  index.html         — rendered cv.html with relative asset paths
  ...
  static/fonts/*     — Inter .woff2 files, copied as-is
...
def main() -> None:
    ...
    html = env.get_template("cv.html").render(pdf_filename=PDF_FILENAME)
    ...
    pdf_bytes = HTML(string=html, base_url=ROOT.as_uri() + "/").write_pdf(
